...
export const getOfflineClaims = async (form_id:number): Promise<IClaim[]> => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    claims.push({
  ...claimRow,
  local_id: claimRow.local_id ?? null, // 🔑 lo recuperamos de SQLite
  answers
});
  }
...
export const startOfflineClaims = (form_id:number) => {
  return async (dispatch: AppDispatch) => {
    ...
    const mappedClaims: IClaim[] = offlineClaims.map(claim => ({
      id: claim.id,
      local_id: claim.local_id ?? null, // 🔑 propagamos el local_id
      status: claim.status,
    ...
    // 🔑 unimos ambos, los unsynced ya traen su id local
    ...
    console.log("Claims leídos desde DB:", allClaims);
    return;
  }
};
